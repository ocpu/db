...
open class DBManyReferenceReverse<F : ActiveRecord, T : ActiveRecord>(
  ...
  override val kClass: KClass<F> by lazy { reverseField.kClass<F>() }

  lateinit var list: DatabaseList<F>

  override fun getValue(thisRef: T, property: KProperty<*>): DatabaseList<F> {
    if (!::list.isInitialized)
      list = DatabaseList(thisRef.database, thisRef, ActiveRecord.getShallowInstance(kClass), reverseField, table, key, referenceKey)
    return list
  }
